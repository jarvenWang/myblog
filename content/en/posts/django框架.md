...
通常是对数据库中的字段值在不获取的情况下进行操作。F 对象内置在数据包django.db.models中，所以使用时需要提前导入
```python
...
相比 F 对象更加复杂一点，它主要应用于包含逻辑运算的复杂查询。Q 对象把关键字参数封装在一起，并传递给 filter、exclude、get 等查询的方法。多个 Q 对象之间可以使用&或者|运算符组合（符号分别表示与和或的关系），从而产生一个新的 Q 对象。当然也可以使用~（非）运算符来取反，从而实现NOT查询。
...
{{<boxmd>}}
&   与操作
|   或操作  Q(market_price__lt=50) | Q(market_price__gt=20)
~   非操作
exact 判断，大小写敏感
contains  是否包含，大小写敏感
...
3 SET_NULL ForeignKey 为null ，null=true
...
### 2、session
生成独立的存储空间（格子）sessionID->借助cookie存储
session要想成:必须有cookie,session数据很难被改，安全
